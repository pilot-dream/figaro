  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
        ...
        {barber.role === 'OWNER' && teamMembers.length > 0 && (
          <div className="space-y-3">
            ...
            <select
              ...
              className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-glass-border text-white text-sm focus:border-figaro-gold-base outline-none appearance-none"
            >
              {teamMembers.map((m) => (
        ...
        {/* Step 1: Services */}
        <div className="space-y-3">
          <label className="text-xs font-bold uppercase text-figaro-gold-base tracking-wider block">
              ...
              return (
                <GlassCard
                  ...
                  className={`p-3.5 flex items-center justify-between border transition-all ${
                    isSelected
                      ? 'border-figaro-gold-base bg-figaro-gold-base'
                      : 'border-glass-border'
                  }`}
                >
                  <div className="flex items-center gap-2.5">
                    <div
                      className={`w-4 h-4 rounded border flex items-center justify-center ${
                        isSelected
                          ? 'bg-figaro-gold-base border-figaro-gold-base text-white'
                          : 'border-white/20'
                      }`}
                    >
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <input
              ...
              className="bg-white/5 border border-glass-border px-3 py-1 rounded-xl text-xs font-bold text-white focus:outline-none focus:border-figaro-gold-base cursor-pointer"
            />
          </div>
                  ...
                  <button
                    ...
                    className={`py-2 px-1 rounded-xl border text-xs font-bold transition-all cursor-pointer ${
                      !slot.available
                        ...
                        : isSelected
                        ? 'bg-figaro-gold-base border-figaro-gold-base text-white shadow-lg'
                        : 'glass-panel text-white hover:border-figaro-gold-base'
                    }`}
                  >
                    {timeStr}
                  </button>
