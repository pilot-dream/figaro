...
export function ClientBottomNav({ onActionClick }: ClientBottomNavProps) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 px-4 pb-[calc(env(safe-area-inset-bottom)+1rem)] pt-2">
      <nav className="bg-figaro-black/85 backdrop-blur-xl border border-white/10 shadow-2xl shadow-black/80 flex items-center justify-around px-8 py-1.5 max-w-lg mx-auto rounded-full relative">
          
          {/* Lado Esquerdo: Agenda */}
          <button 
            onClick={() => navigate('/meus-agendamentos')} 
            className={`flex flex-col items-center justify-center min-w-[60px] transition-colors cursor-pointer ${isSchedule ? 'text-figaro-gold-base font-semibold' : 'text-gray-400 hover:text-white'}`}
          >
             <Calendar className="w-6 h-6" />
             <span className="text-[10px] font-medium mt-1">Agenda</span>
             {isSchedule && <div className="w-1 h-1 bg-figaro-gold-base rounded-full mx-auto mt-1 shadow-sm shadow-figaro-gold-base/30" />}
          </button>
          ...
          <button 
            ...
            className={`flex flex-col items-center justify-center min-w-[60px] transition-colors cursor-pointer ${isProfile ? 'text-figaro-gold-base font-semibold' : 'text-gray-400 hover:text-white'}`}
          >
             <UserIcon className="w-6 h-6" />
             <span className="text-[10px] font-medium mt-1">Perfil</span>
             {isProfile && <div className="w-1 h-1 bg-figaro-gold-base rounded-full mx-auto mt-1 shadow-sm shadow-figaro-gold-base/30" />}
          </button>
